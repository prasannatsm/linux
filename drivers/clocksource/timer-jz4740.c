...
static int __init jz4740_timer_init(struct device_node *node)
{
	int ret;
	uint32_t clk_rate;
	uint16_t ctrl;
	...
	jz4740_timer_base = of_iomap(node, 0);
	...
	jz4740_clockevent.irq = irq_of_parse_and_map(node, 0);
...
TIMER_OF_DECLARE(jz4740, "ingenic,jz4740-timer", jz4740_timer_init);
